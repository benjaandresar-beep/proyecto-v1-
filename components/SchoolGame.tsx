...
// ---------- contenido provisional (el definitivo lo escribe el responsable clínico) ----------
...
interface NpcDef {
  ...
  pelo: string;
  polera: string;
  x: number; // % del ancho de la escena
  y: number; // % del alto de la escena (pies del personaje)
}

const COMPANEROS: NpcDef[] = [
  { id: "martin", nombre: "Martín", piel: "#e8b88a", pelo: "#2b2b2b", polera: "#e0503f", x: 14, y: 78 },
  { id: "sofia", nombre: "Sofía", piel: "#f5d3b3", pelo: "#8a5a2b", polera: "#f2c24b", x: 32, y: 90 },
  { id: "tomas", nombre: "Tomás", piel: "#c98e5f", pelo: "#5b4232", polera: "#67c08a", x: 50, y: 70 },
  { id: "emilia", nombre: "Emilia", piel: "#9c6a43", pelo: "#2b2b2b", polera: "#9a6fd0", x: 68, y: 88 },
  { id: "lucas", nombre: "Lucas", piel: "#e8b88a", pelo: "#c98f3d", polera: "#4a9fd6", x: 86, y: 74 },
  { id: "maite", nombre: "Maite", piel: "#f5d3b3", pelo: "#d6593f", polera: "#ee8fbb", x: 60, y: 60 },
  { id: "diego", nombre: "Diego", piel: "#c98e5f", pelo: "#2b2b2b", polera: "#f2994a", x: 8, y: 92 },
  { id: "anto", nombre: "Anto", piel: "#e8b88a", pelo: "#5b4232", polera: "#56c5c5", x: 90, y: 92 },
];
...
// 7 burbujas iniciales: varias displacenteras + algunas neutras/reguladoras (salvaguarda 5.2)
const BURBUJAS_INICIALES: Record<string, EmotionId> = {
  martin: "enojo",
  sofia: "tristeza",
  tomas: "alegria",
  emilia: "verguenza",
...
const LINEAS_NPC: Record<EmotionId, string> = {
...
const CARGA_POR_BURBUJA = 18;
const CARGA_RUIDO = 8;
const ALIVIO_AMIGO_CALMA = 8;
const ALIVIO_EJERCICIO = 20;
const RESPAWN_BURBUJA_MS = 25000;
...
function sumarCarga(carga: ChargeMap, emocion: EmotionId, cantidad: number): ChargeMap {
...
function reducirTotal(carga: ChargeMap, cantidad: number): ChargeMap {
  const total = totalCharge(carga);
...
function escalarA(carga: ChargeMap, objetivo: number): ChargeMap {
  const total = totalCharge(carga);
  if (total <= 0) return carga;
...
export default function SchoolGame() {
  ...
  return (
    <div className="escena-envoltorio" onClick={moverJugador}>
      ...
      >
        <NpcSvg piel={PROFE.piel} pelo={PROFE.pelo} polera={PROFE.polera} esAdulto />
      ...
      {COMPANEROS.map((npc) => {
        ...
        return (
          ...
          >
            ...
            <NpcSvg piel={npc.piel} pelo={npc.pelo} polera={npc.polera} />
